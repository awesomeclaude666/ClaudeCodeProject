...
DELAY_RANGES = {
    "between_pushes": (30, 60),
    "between_posts": (120, 300),
    "between_push_and_post": (60, 180),
    "pre_action_pause": (2, 5),
    "post_action_pause": (3, 8),
    "session_start": (3, 10),
}
...
STATE_FILE = "/tmp/ptt_global_safety.json"
...
def cmd_check(args):
    ...
    # 檢查每日上限
    action_key = "pushes" if action == "push" else "posts"
    ...
    if state["last_action"]:
        ...
        if elapsed < MIN_SECONDS_BETWEEN_ACTIONS:
            ...
            wait_seconds += random.uniform(5, 20)

    if action == "push":
        delay_range = DELAY_RANGES["between_pushes"]
    else:
...
def main():
    parser = argparse.ArgumentParser(description="PTT 全域安全模組")
    subparsers = parser.add_subparsers(dest="command", required=True)

    p_check = subparsers.add_parser("check", help="預檢是否允許操作")
    p_check.add_argument("--action", required=True, choices=["push", "post"])

    p_record = subparsers.add_parser("record", help="記錄操作結果")
    p_record.add_argument("--action", required=True, choices=["push", "post"])
